...
const Submission = () => {
  ...
  const loadPersistedState = () => ({
    ...
    currentSubIndex: parseInt(localStorage.getItem('currentSubQuestionIndex')) || 0,
    ...
    attemptedQuestions: JSON.parse(localStorage.getItem('attemptedQuestions')) || [],
  ...
  const [currentSubQuestionIndex, setCurrentSubQuestionIndex] = useState(persistedState.currentSubIndex);
  const [attemptedQuestions, setAttemptedQuestions] = useState(persistedState.attemptedQuestions);
  ...
  const persistState = () => {
    storageHelpers.saveState(LOCAL_STORAGE_KEYS.CURRENT_QUESTION, currentQuestionIndex);
    storageHelpers.saveState(LOCAL_STORAGE_KEYS.CURRENT_SUB_QUESTION, currentSubQuestionIndex);
    storageHelpers.saveState(LOCAL_STORAGE_KEYS.ANSWERS, {
        ...storageHelpers.loadState(LOCAL_STORAGE_KEYS.ANSWERS, {}),
        [currentQuestionIndex]: selectedAnswer
    });
    storageHelpers.saveState(LOCAL_STORAGE_KEYS.SUB_ANSWERS, selectedSubAnswers);
  ...
  const handlePrevious = () => {
    ...
    if (isNextGen && currentSubQuestionIndex > 0) {
      // If we're in a case study and not at the first sub-question,
      // go to previous sub-question
      setCurrentSubQuestionIndex((prev) => prev - 1);
      setSubmitted(false);

      // Load previous sub-question answer if it exists
      const prevSubAnswer = selectedSubAnswers[currentSubQuestionIndex - 1];
      if (prevSubAnswer) {
        setSelectedSubAnswers((prev) => ({
          ...prev,
          [currentSubQuestionIndex - 1]: prevSubAnswer,
        }));
      }
    } else if (currentQuestionIndex > 0) {
      // If we're at the first sub-question or it's a traditional question,
      // go to previous main question
      const prevQuestion = questions[currentQuestionIndex - 1];
      setCurrentQuestionIndex((prev) => prev - 1);

      if (prevQuestion.type === "nextgen") {
        // If previous question is case study, go to its last sub-question
        setCurrentSubQuestionIndex(prevQuestion.Questions.length - 1);

        // Load previous answers if they exist
        const prevSubAnswer =
          selectedSubAnswers[prevQuestion.Questions.length - 1];
        if (prevSubAnswer) {
          setSelectedSubAnswers((prev) => ({
            ...prev,
            [prevQuestion.Questions.length - 1]: prevSubAnswer,
          }));
        }
      } else {
        // If previous question is traditional
        setCurrentSubQuestionIndex(0);
        setSelectedAnswer(
          persistedState.answers[currentQuestionIndex - 1] || ""
        );
      }
      setSubmitted(false);
    }
  ...
  const handleSubmit = () => {
    const currentQuestion = questions[currentQuestionIndex];
    const questionKey = `${currentQuestionIndex}${isNextGen ? `-${currentSubQuestionIndex}` : ''}`;
                ...
                default:
                  return (
                    <div className="sub-question">
                      <h4>{subQuestion?.question}</h4>
                      <p>
                        Question format "{subQuestion?.format}" display not
                        implemented yet.
                      </p>
                    </div>
                  );
